/**
 * 基于puppeteer-lottie改写了,
 * 不使用puppeteer而是使用puppeteer-core，
 * 调用系统自己的GoogleChrome来进行渲染，
 * 以减少包大小以及解决依赖问题
 */
'use strict'
...
// Instead of normal require，to get compiled successfully in nodec
...
// 指定本机Chrome作为渲染器
const executablePath = require('chrome-location');
if (!executablePath) {
  log.error(`当前系统找不到Google Chrome，请确保chrome已正确安装`);
  process.exit(1);
} else {
  log.warn(`当前系统Google Chrome路径为${executablePath}\n`)
}
